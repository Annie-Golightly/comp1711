 ...
 * @param delimiter the character which deliminates (splits up) the data - a comma in this case
 * @param pos a pointer to where it should store the position - function can only return one thing so you need to use pointers to store info
...
int main()
{
    SONG_DATA songs[100]; // making 100 song_data structs in an array called songs
    int buffer_size = 250;
    char line[buffer_size];
    int counter=0;

    FILE *input = fopen("song_data.txt", "r"); // opening file for reading
    ...
    {   
        ...
        tokeniseRecord(line,",",pos,artist,title,year); // calls the function to split up the line, storing it in the temp variables
        // but then what??
        strcpy(songs[counter].artist, artist); // puts data into array of structs
